...
class TopologyCleanAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        p_val.setHelp(
            "Считает количество некорректных геометрий на входе и на выходе.\n"
...
class InsertNodesAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        p = QgsProcessingParameterNumber(
            self.MAX_PASSES, tr("Предельное количество проходов"),
        ...
        p.setHelp(
            ...
            "не является покрытием. Причина обычно в наложении объектов."
        )
        self.addParameter(p)
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if stats["passes"] >= max_passes and stats["nodes_inserted"]:
            feedback.pushWarning(
                tr("Предел количества проходов достигнут, узлы могут быть ещё нужны. "
