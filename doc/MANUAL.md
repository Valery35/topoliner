# Topoliner. Краткое руководство

[English version](MANUAL.en.md)

Версия 0.13.0

Плагин добавляет в панель Processing группу **Topoliner** с одиннадцатью
инструментами и кнопку врезки на панель инструментов. Основная задача -
привести полигональное покрытие в порядок без ручного прилипания слоя
к самому себе.
...
Работает на QGIS от 3.16 до 4.0 включительно.
...
группа **1. Топология** содержит 1.01 - 1.08, группа
**2. Генерализация** содержит 2.01 - 2.03. Порядок в первой группе
...
| **1.08 Врезка контуров в покрытие** | Врезает контур, кромка разреза достаётся обеим сторонам одинаковой | Да, в новый слой |
...
| **2.03 Топологическая модель покрытия** | Раскладывает покрытие на узлы и дуги | Нет |

Все инструменты работают в моделях и в пакетном режиме. Исходный слой
не изменяется никогда, результат всегда новый слой. У врезки есть ещё
кнопка на панели инструментов, она правит открытый слой в режиме
редактирования.
...
на главный вопрос, сколько нарушений уйдёт молча и сколько придётся разбирать
вручную.
...
должно остаться пусто. Всё, что осталось в разряде **review**, разбирается вручную.
...
**Шаг 4.** При необходимости **1.07 Контроль сборки по атрибуту**. Проверяется, собирается ли
...
Уменьшать допуск до ширины самого узкого объекта не требуется.
...
**Подсказка по допуску.** После проверки инструмент печатает, как расхождения
распределены в этом слое: сколько их, медиана, девяносто пятый процентиль
и максимум. Если в распределении есть разрыв, он называется. До него
погрешность оцифровки, за ним разногласие между источниками. Если разрыва нет,
инструмент так и говорит, а не выдумывает число.

Отдельно печатается потолок, то есть величина, выше которой допуск брать
не следует. Такой допуск схлопнет короткие рёбра и узкие объекты. Считается по пятому
процентилю длины ребра и минимальной ширине объекта.

Если медиана расхождений оказалась близка к заданному допуску, распределение
обрезано. Дальше допуска проверка не смотрит, и настоящие расхождения крупнее.
Стоит повторить с допуском покрупнее.
...
означает лишь близость, и количество таких находок задаётся допуском. Если в сводке
...
Порядок зафиксирован и не настраивается. Иглы снимаются до сшивки, иначе они
превращаются в ложные узлы и разносятся по соседям. Проверка корректности идёт
после сшивки, потому что сшивка сама способна породить самопересечение.
...
по количеству вставленных узлов.
...
| Допуск (в единицах CRS слоя) | 2 | Предельное смещение вершины |
| Режим | слияние вершин и вставка узлов | Другие варианты: только вставка узлов, вершины при этом не двигаются, и только слияние вершин |
| Эталонный слой, *необязательный* | - | Его вершины неподвижны и притягивают входной слой. Сам не изменяется |
| Кто кого притягивает | крупные объекты притягивают мелкие | Второй вариант: по порядку объектов в слое |
...
| Если сшивка испортила геометрию | исправить, а если не выходит, вернуть исходную | Другие варианты: сразу вернуть исходную геометрию объекта либо оставить как есть |
...
до и после. Если строки о равенстве нет, результат неверен.
...
находятся, и печатает их количество. На типичном слое кригинга хватает трёх.

**Отклонение от ребра** это не допуск. Расстояние здесь равно нулю, а величина
нужна лишь как защита от ложных срабатываний. Умолчание отвечает точности
координат. Задавать метры не следует. Инструмент начнёт подтягивать рёбра,
и гарантия неизменной площади перестанет выполняться.
...
| Ставить узлы в точках пересечения рёбер | да | |
| Предельное количество проходов | 10 | На согласованном покрытии хватает двух-трёх. Достижение предела означает, что слой не является покрытием |
| Слой с узлами | - | |
...
**Для линий** проверяется то же самое, собирается ли группа в одну связную
цепь. Смежные участки склеиваются, поэтому количество исходных отрезков роли
не играет.
...
## 1.08 Врезка контуров в покрытие

Врезает контуры в полигональное покрытие. Соседи урезаются, врезанный контур
становится отдельным объектом. Результат это новый слой.

**Зачем.** Врезка средствами редактора делает два независимых действия.
У нового объекта отрезается то, что легло на соседей, а у каждого соседа
отрезается то, что легло под новый объект. Кромка разреза приходит из двух
разных вычислений и совпадает только на вид. Вершины расходятся, и дальше эти
расхождения находит инструмент 1.01.

Здесь кромка приходит из одного наложения и достаётся обеим сторонам
одинаковой. Это проверено тестами через модель покрытия. Кромка становится
одной дугой с двумя соседями, а не двумя дугами рядом.

**Недостающие узлы.** Разрез кончается на границе соседа с третьим объектом,
и у этого третьего узла в той точке нет. Инструмент достраивает такие узлы так
же, как это делает 1.06. Узел ставится в проекцию точки на ребро, поэтому форма
и площадь третьего объекта не меняются.

**Порог площади.** Остаток соседа мельче порога отходит контуру целиком,
осколок не создаётся. Кусок соседа мельче порога контуру не достаётся, и сосед
остаётся как был.

**Атрибуты нового объекта.** Поле, у которого все отдавшие площадь соседи имеют
одно значение, получает это значение. Поле с разными значениями остаётся
пустым. Контур, легший на пустое место, получает пустые атрибуты целиком.
Ключ слоя не переносится никогда, иначе новый объект повторил бы ключ соседа.

**Порядок.** Контуры применяются по одному, в порядке слоя. Результат каждой
врезки входит в покрытие до начала следующей, поэтому два перекрывающихся
контура не накладываются друг на друга.

**Отметки Z** берутся с ближайшего исходного ребра. Вершина на середине ребра
получает отметку, посчитанную вдоль этого ребра. За краем покрытия исходных
рёбер нет, и там отметка достаётся от ближайшего края.

**Линии** врезаются после построения буфера. Буфер строится отдельным
инструментом QGIS.

**Площадь.** Врезка площадь только перераспределяет. Прирост покрытия равен
площади контуров, легшей за его краем. Отчёт печатает обе величины и при
расхождении выдаёт предупреждение.
...
| Параметр | Умолчание | Что задаёт |
|---|---|---|
| Покрытие (полигоны) | - | Слой не изменяется |
| Врезаемые контуры (полигоны) | - | Контуры, по одному на объект слоя |
| Порог площади (в кв. единицах CRS) | 1 | Ниже этой площади остаток отходит контуру, а мелкий кусок не отрезается |
| Допустимое отклонение вершины от ребра | 1e-06 | Вставка недостающих узлов. Ноль отключает |
| Восстанавливать отметки Z | да | Отметка берётся с ближайшего исходного ребра |
| Покрытие с врезкой | - | Выходной слой |

### Кнопка на панели инструментов

Врезку применяют и в режиме редактирования, где окно Processing неудобно.
Кнопка **Врезка контура** включает рисование по карте. Левая кнопка ставит
вершину, правая завершает контур, Backspace снимает последнюю вершину,
Esc отменяет рисование. Вершина прилипает к тому, к чему настроено прилипание
в проекте.

Контур врезается в активный слой. Слой должен быть слоем полигонов и стоять
в режиме редактирования. Правка идёт одной командой, поэтому отмена в QGIS
возвращает покрытие целиком.

Пороги берутся из окна **Параметры врезки**, кнопка которого стоит рядом.
Это те же величины, что у инструмента 1.08.

Если слой однокомпонентный, а остаток соседа распался на части, первая часть
остаётся за объектом, остальные становятся отдельными объектами с теми же
...
Он держит углы и характерные изломы, но на плавной кривой оставляет заметные
...
Допуск в обоих случаях задаётся в единицах длины, одним полем. Одно поле работает
и в моделях, и в пакетном режиме. Точного соответствия между методами нет,
один меряет расстояние, другой площадь. При равной величине в поле Висвалингам
оставляет заметно больше вершин. Для сравнимого результата его допуск
берут крупнее.
...
удваивает количество вершин, обычно хватает одного или двух.

Сглаживание единственное место, где повторный запуск по результату меняет
геометрию. Каждый прогон срезает углы заново. Прореживание без сглаживания
при повторе ничего не меняет.
...
не портится. Проверьте результат инструментом 1.01, количество находок не должно
вырасти.
...
| Сглаживание, количество проходов | 0 | Срезание углов после прореживания. Каждый проход примерно удваивает количество вершин |
...
границу дважды, по разу от каждого соседа. Это две совпадающие линии, одна поверх
...
Раскладывает покрытие на узлы и дуги. Слой не изменяется, на выходе два
новых: точки узлов и линии дуг.

**Что это такое.** Та самая модель, что была в ArcInfo coverage. Полигон
описывается не своей границей, а ссылками на дуги. Дуга хранится один раз
и знает, что лежит слева и справа. Правка дуги меняет обоих соседей сразу,
поэтому границам просто негде разойтись.

**Узлы** несут степень, то есть количество сходящихся дуг. Степень 1 это висячий
конец, 2 псевдоузел, который можно убрать слиянием дуг, 3 и больше настоящее
ветвление, где сходятся три и более объекта.

**Дуги** несут номера узлов начала и конца и идентификаторы обоих соседей.
Для края покрытия второй сосед равен минус единице.

**Чем отличается от 2.02.** Инструмент 2.02 выдаёт границы для оформления.
Это линии с признаком того, с чем они граничат. Здесь помимо линий выдаются узлы
со степенью, и дуги привязаны к ним номерами. Это модель для разбора
связности, а не для стилизации.

Замер на слое зон: 341 объект дал 566 узлов и 899 дуг, из них 708 между
объектами. На кадастровом слое из 4234 объектов вышло 7499 узлов и 10869 дуг,
...
|---|---|---|
| Входной слой (полигоны) | - | Слой не изменяется |
| Отклонение при поиске общих вершин | 0.000001 | Узлы достраиваются перед разбором, вершины не двигаются |
| Точность опознания общих вершин | 0.000001 | |
| Узлы | - | Выходной слой точек |
| Дуги | - | Выходной слой линий |
...
поле заполнено исходными значениями, а не пустое. Ограничение здесь одно.
...
**4. Если сервер отверг явные значения IDENTITY.** Откатить правку и идти через временную
...
**Слой читается в память целиком.** Замеры на кадастровом слое: 11 866
объектов, 15 700 колец, 726 тысяч вершин.

| Инструмент | Время |
|---|---|
| 1.01 Проверка топологии, допуск 2 | 48 с |
| 1.06 Вставка недостающих узлов | 84 с |
| 2.01 Топологическое упрощение, допуск 5 | 7 с |
| 2.03 Топологическая модель покрытия | 96 с |
...
Имена полей латинские и короткие, потому что по ним пишут выражения и фильтры.
У каждого поля есть псевдоним на языке интерфейса. Он виден как подпись столбца
в таблице атрибутов и в форме объекта, а в выражениях по-прежнему работает имя
поля из таблиц ниже.

Если результат записан в GeoPackage, подписи уходят в сам файл. Такой слой
показывает их и в другом проекте, и у другого человека. Записывается язык
интерфейса на момент прогона. Остальные форматы второго имени поля не хранят,
там подпись живёт только в проекте.

Измеренные величины округляются до четырёх значащих цифр. Округление идёт
по значащим цифрам, а не по знакам после запятой. Поэтому площадь в микроны
не превращается в ноль, а длина в тысячи метров не несёт знаков, которых
в исходных данных нет. Количества не округляются вовсе.
...
|---|---|---|
| `num` | целое | Сквозной номер находки. Совпадает с номером в отчёте списком |
...
| `value` | вещественное | Измеренная величина: площадь перекрытия или щели, расстояние до соседней линии, количество снятых вершин. Смысл зависит от типа |
...
**Отчёт списком.** Инструмент 1.01 может выгрузить находки текстовым файлом:
номер, что не так, у каких объектов, где именно. Номер совпадает с полем `num`
в слое, поэтому по списку место ищется на карте, а по идентификатору объекта
в исходных данных. Список удобно передать тому, кто данные готовил.
...
Выдаёт инструмент **2.03**.

| Поле | Тип | Что содержит |
|---|---|---|
| `node_id` | целое | Номер узла, на него ссылаются дуги |
| `degree` | целое | Сколько дуг сходится в узле |
| `kind` | строка | Висячий при степени 1, псевдоузел при 2, ветвление при 3 и больше |
...
| Поле | Тип | Что содержит |
|---|---|---|
| `arc_id` | целое | Номер дуги |
| `from_node` | целое | Узел начала |
| `to_node` | целое | Узел конца |
| `left_fid` | целое | Объект слева от дуги по её направлению |
| `right_fid` | целое | Объект справа, либо -1 для края покрытия |
| `length` | вещественное | Длина дуги |
...
Три плагина решают соседние задачи и работают вместе.

- **[Isoliner](https://plugins.qgis.org/plugins/grid_isolines/)** - кригинг,
  изолинии и контурные полигоны, гидрология рельефа, разрезы по скважинам.
  [Исходники](https://github.com/Valery35/qgis-isoliner)
- **[Isoliner3D](https://plugins.qgis.org/plugins/isoliner3d/)** - автономный
  трёхмерный просмотр поверхностей, тел пластов, скважин и полиэдров, подсчёт
  запасов по блочной модели.
  [Исходники](https://github.com/Valery35/qgis-isoliner3d)

Изолинии и границы, построенные в Isoliner, приводятся в порядок Topoliner,
а результат смотрится в Isoliner3D.
