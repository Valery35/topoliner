...
Плагин добавляет в панель Processing группу **Topoliner** с шестью
...
группа **1. Топология** содержит 1.01 - 1.05, группа
...
| **1.02 Топологическая очистка (всё сразу)** | Чинит всё, что является мусором | Да, в новый слой |
| **1.03 Топологическая сшивка (узлы и вершины)** | Только согласование узлов и вершин | Да, в новый слой |
| **1.04 Контроль сборки по атрибуту** | Проверяет, собираются ли блоки и панели в целое тело | Нет |
| **1.05 Вставить недостающие узлы** | Достраивает узлы, не меняя ни формы, ни площади | Да, в новый слой |
...
Три шага, порядок именно такой.

**Шаг 1. Проверить.** Запустить **1.01 Проверка топологии** с допуском 2 и порогом
площади 1. Посмотреть сводку в панели Processing. Она отвечает на главный
вопрос: сколько нарушений уйдёт молча и сколько придётся смотреть руками.

**Шаг 2. Почистить.** Запустить **1.02 Топологическая очистка (всё сразу)** с теми же
...
**Шаг 4. Проверить сборку.** Запустить **1.04 Контроль сборки по атрибуту** по полю
блока, затем по полю панели. Это конечный критерий приёмки: ради корректной
сборки всё и делается. Если все группы собираются в одно тело, покрытие готово.
...
объекта.** Если объект уже допуска, его противоположные берега слипнутся,
...
## 5. Инструмент 1.01 Проверка топологии
...
и посмотрите распределение. Провал в распределении и есть правильный порог
...
**Площадь законной полости.** Целик, озеро или незакартированный участок это
...
## 6. Инструмент 1.02 Топологическая очистка (всё сразу)
...
- **Восстанавливать отметки Z** - см. раздел 9.
...
## 7. Инструмент 1.03 Топологическая сшивка
...
мелкие. По смыслу обычно правильнее второе.

---

## 7а. Инструмент 1.04 Контроль сборки по атрибуту
...
дальше него, считаются законно отдельными телами, а разрывом остаётся только
разделение внутри тела. Полости в таких данных тоже обычно законны, для них есть
...
## 7б. Инструмент 1.05 Вставить недостающие узлы
...
## 7в. Инструмент 2.01 Топологическое упрощение
...
## 8. Что решено по общим границам
...
## 10а. Заливка результата обратно в MSSQL
...
**Слой читается в память целиком.** Для слоёв в сотни тысяч объектов
обрабатывайте по частям.

**Инструменты топологии работают с полигонами.** Сшивка работает и с линиями,
остальные проверки нет.
...
160 тестов, QGIS для них не нужен. Проверки перекрытий и щелей тестируются
